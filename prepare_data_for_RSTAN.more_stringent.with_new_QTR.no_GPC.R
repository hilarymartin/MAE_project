...
"QTR/duoHMM_results/duos_removed_from_QTR370_and_QTRCoreExome_due_to_overlap_with_other_chips.more_stringent.generr_removed.recombination_count.p_0.5.annotated.double_xovers_within_X_SNPs_removed.with_new_QTR.no_GPC.txt",quote=F,sep="\t")


write.table(removed,"samples_removed_due_to_missing_or_suspicious_age_data.more_stringent.generr_removed.recombination_count.p_0.5.annotated.double_xovers_within_X_SNPs_removed.with_new_QTR.no_GPC.txt",quote=F,sep="\t")
...
write.table(all.data,"all_cohorts.post_MERLIN.generr_removed.recombination_count.p_0.5.annotated.double_xovers_within_X_SNPs_removed.more_stringent.with_new_QTR.no_GPC.txt",quote=F,sep="\t",row.names=F)
...
write.table(mat.key.model0,"RSTAN_output/key_for_maternal_cohorts_to_include_in_model_1.informative_nuclear_families_only.more_stringent.with_new_QTR.no_GPC.txt",quote=F,sep="\t",row.names=F)
write.table(pat.key.model0,"RSTAN_output/key_for_paternal_cohorts_to_include_in_model_1.informative_nuclear_families_only.more_stringent.with_new_QTR.no_GPC.txt",quote=F,sep="\t",row.names=F)
...
write.table(mat.key.model1,"RSTAN_output/key_for_maternal_cohorts_to_include_in_model_1.more_stringent.with_new_QTR.no_GPC.txt",quote=F,sep="\t",row.names=F)
write.table(pat.key.model1,"RSTAN_output/key_for_paternal_cohorts_to_include_in_model_1.more_stringent.with_new_QTR.no_GPC.txt",quote=F,sep="\t",row.names=F)
...
#informative meioses for model 3 (p  parameter will control mu_c)
data1.mat3=list(y=data1.mat$nrec,cohort=as.integer(as.factor(data1.mat$cohort)),cohort2=as.integer(as.factor(data1.mat$cohort)),family=as.integer(as.factor(data1.mat$PARENT)),Age=as.numeric(data1.mat$age.at.birth),J=nrow(data1.mat),
        I=length(unique(data1.mat$PARENT)),    C=length(unique(data1.mat$cohort)),C2=length(unique(data1.mat$cohort)),alpha_parameters=rep(10,length(unique(data1.mat$cohort))),beta_parameters=rep(1,length(unique(data1.mat$cohort))))

data1.pat3=list(y=data1.pat$nrec,cohort=as.integer(as.factor(data1.pat$cohort)),cohort2=as.integer(as.factor(data1.pat$cohort)),family=as.integer(as.factor(data1.pat$PARENT)),Age=as.numeric(data1.pat$age.at.birth),J=nrow(data1.pat),
        I=length(unique(data1.pat$PARENT)),    C=length(unique(data1.pat$cohort)),C2=length(unique(data1.pat$cohort)),alpha_parameters=rep(10,length(unique(data1.pat$cohort))),beta_parameters=rep(1,length(unique(data1.pat$cohort))))
...
write.table(variance.in.mean.by.family,"variance_in_mean_duoHMM_count_by_parent.informative_meioses_only.min_2_kids.more_stringent.with_new_QTR.no_GPC.txt",quote=F,sep="\t")
##### Model 2
...
write.table(mat.key.model2,"RSTAN_output/key_for_maternal_cohorts_to_include_in_model_2.more_stringent.with_new_QTR.no_GPC.txt",quote=F,sep="\t",row.names=F)
write.table(pat.key.model2,"RSTAN_output/key_for_paternal_cohorts_to_include_in_model_2.more_stringent.with_new_QTR.no_GPC.txt",quote=F,sep="\t",row.names=F)
...
write.table(family.type.count.multikid,"number_of_duos_by_cohorts.families_with_multiple_children.with_new_QTR.no_GPC.txt",quote=F,sep="\t")
write.table(family.type.count.inf,"number_of_duos_by_cohorts.informative_duos.with_new_QTR.no_GPC.txt",quote=F,sep="\t")
write.table(family.type.count.inf.nuc,"number_of_duos_by_cohorts.informative_nuc_fam_duos.with_new_QTR.no_GPC.txt",quote=F,sep="\t")
...
write.table(mat.key.model3,"RSTAN_output/key_for_maternal_cohorts_to_include_in_model_3.more_stringent.with_new_QTR.no_GPC.txt",quote=F,sep="\t",row.names=F)
write.table(pat.key.model3,"RSTAN_output/key_for_paternal_cohorts_to_include_in_model_3.more_stringent.with_new_QTR.no_GPC.txt",quote=F,sep="\t",row.names=F)
...
write.table(mat.key2.model3,"RSTAN_output/key_for_maternal_cohorts_to_include_in_model_3.more_stringent.with_new_QTR.no_GPC.original_cohorts.txt",quote=F,sep="\t",row.names=F)
write.table(pat.key2.model3,"RSTAN_output/key_for_paternal_cohorts_to_include_in_model_3.more_stringent.with_new_QTR.no_GPC.original_cohorts.txt",quote=F,sep="\t",row.names=F)
...
write.table(mat.beta.parameters,"parameters_for_beta_distribution.model3_maternal.more_stringent.with_new_QTR.no_GPC.txt",col.names=F,row.names=F,sep="\t",quote=F)
write.table(pat.beta.parameters,"parameters_for_beta_distribution.model3_paternal.more_stringent.with_new_QTR.no_GPC.txt",col.names=F,row.names=F,sep="\t",quote=F)
...
save.image("duoHMM_data_for_RSTAN.more_stringent.with_new_QTR.no_GPC.RData")
