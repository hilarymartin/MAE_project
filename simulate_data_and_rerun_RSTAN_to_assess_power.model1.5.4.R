...
pdf(paste0(mydir,"/coverage_plot_model1.5.4.maternal.exponentiated.pdf"),height=4,width=5,useDingbats=FALSE)
...
#    plot(myx,coverage.by.bin[,paste0("coverage_beta_age_",quant)]*100,col=mycolors[i],xlab="center of beta_Age bin",ylab="Coverage (%)",ylim=c(0,100),xlim=range(mybins),type="b",pch=20,main="Model 2")
          plot(myx,coverage.by.bin[,paste0("coverage_beta_age_",quant)]*100,col=mycolors[i],xlab="center of beta_Age bin",ylab="Coverage (%)",ylim=c(0,100),xlim=range(exp(mybins)),type="b",pch=20,main="Model 2")
} else {
...
pdf(paste0(mydir,"/power_plot_model1.5.4.maternal.exponentiated.pdf"),height=4,width=5,useDingbats=FALSE)
#myx=mybins[1:(length(mybins)-1)]+(mybins[2:length(mybins)]-mybins[1:(length(mybins)-1)])/2
myx=exp(mybins[1:(length(mybins)-1)]+(mybins[2:length(mybins)]-mybins[1:(length(mybins)-1)])/2)
for(i in 1:length(myquantiles)){
  quant=myquantiles[i]
  if(i==1){
      ...
      plot(myexp,100-coverage.by.bin[,paste0("coverage_0_",quant)]*100,col=mycolors[i],xlab="center of beta_Age bin",ylab="Power (%)",ylim=c(0,100),xlim=range(exp(mybins)),type="b",pch=20,main="Model 2")
} else {
