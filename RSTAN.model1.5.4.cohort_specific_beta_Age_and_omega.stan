data {
  ...
  transformed parameters {
    ...
    for(j in 1:J){
        ...
        mu[j] <-  exp(0.1*(exp_a0_bychild[j]+beta_Age[cohort[j]]*Age[j])) / (omega[cohort[j]] - 1); ///needed to keep in exp(0.1*...) to ensure that mu > 0, since shape parameter of neg binomial must be >0
    }
  }
  model {
  //Priors
