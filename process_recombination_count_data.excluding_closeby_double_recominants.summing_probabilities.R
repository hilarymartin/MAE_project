...
window=as.numeric(args[4]) #distance between which double recombinants should be excluded
...
maternal.counts=apply(maternal[,2:3],1,function(indivs){sum(recomb3[recomb3[,1]==indivs[1] & recomb3[,2]==indivs[2],"PROB_RECOMBINATION"])})
...
paternal.counts=apply(paternal[,2:3],1,function(indivs){sum(recomb3[recomb3[,1]==indivs[1] & recomb3[,2]==indivs[2],"PROB_RECOMBINATION"])})
...
write.table(maternal,paste(out.prefix,".total_recombination_counts.maternal.summing_probabilities.no_double_recombinants_within_",format(window,scientific=F),"bp.txt",sep=""),quote=F,sep="\t",row.names=F)
write.table(paternal,paste(out.prefix,".total_recombination_counts.paternal.summing_probabilities.no_double_recombinants_within_",format(window,scientific=F),"bp.txt",sep=""),quote=F,sep="\t",row.names=F)
