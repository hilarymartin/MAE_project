  data { 
    ...
    int mother[J]; // indicates which mother is with each child
  ...
  transformed parameters {
      ...
      vector[J] mu;
      vector[C] actual_beta_Age;
      vector[C-1] actual_beta_Cohort;
      vector[J] my_beta_Cohort;
      actual_beta_Age <- 0.1 * beta_Age;
      actual_beta_Cohort <- 0.1 * beta_Cohort;
      omega <- 1 / inv_omega;
      for(j in 1:J){
          exp_a0_bychild[j] <- exp_a0[mother[j]];
         if(cohort[j]==1){
              my_beta_Cohort[j] <- 0;
         } else {
