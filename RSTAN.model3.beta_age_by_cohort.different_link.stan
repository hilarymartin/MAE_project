  ...
  transformed parameters {
      ...
      for(j in 1:J){
        ...
        mu[j] <-  exp(1*(exp_a0_bychild[j]+beta_Age[cohort2[j]]*Age[j])) / (omega - 1); ///needed to keep in exp(0.1*...) to ensure that mu > 0, since shape parameter of neg binomial must be >0
      }
  }
  model {
