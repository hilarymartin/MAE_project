...
counts=read.delim("../maternal_crossovers.informative_meioses.with_twins_indicated.txt",header=T,stringsAsFactors=F)
...
qtr.counts=counts[counts$cohort %in% c("QTR370","QTR610"),]
...
qtr.counts.non.art=qtr.counts[(qtr.counts$ANY_ART=="n" & !is.na(qtr.counts$ANY_ART)) |
                                  ...
                                  (!qtr.counts$old.any.ART & qtr.counts$old.answered.any.ART) | (qtr.counts$kids.preIVF==1 & !is.na(qtr.counts$kids.preIVF)),]
...
load("../duoHMM_data_for_RSTAN.more_stringent.no_GPC.RData")
...
write.table(mat.key.model1,"../RSTAN_output/key_for_maternal_cohorts_to_include_in_model_1.more_stringent.no_GPC.ART_separated.txt",quote=F,sep="\t",row.names=F)
...
save.image("../duoHMM_data_for_RSTAN.more_stringent.no_GPC.ART_separated.RData")
...
mat.key.model1=unique(data.frame(as.integer(as.factor(data1.mat$cohort2)),as.factor(data1.mat$cohort2)))
colnames(mat.key.model1)=c("Code","Cohort")
write.table(mat.key.model1,"../RSTAN_output/key_for_maternal_cohorts_to_include_in_model_1.more_stringent.no_GPC.NTR_pill_separated.txt",quote=F,sep="\t",row.names=F)
...
data1.mat2$cohort_by_family = cohort_by_family
save.image("../duoHMM_data_for_RSTAN.more_stringent.no_GPC.NTR_pill_separated.RData")
...
mat.key.model1=unique(data.frame(as.integer(as.factor(data1.mat$cohort2)),as.factor(data1.mat$cohort2)))
colnames(mat.key.model1)=c("Code","Cohort")
write.table(mat.key.model1,"../RSTAN_output/key_for_maternal_cohorts_to_include_in_model_1.more_stringent.no_GPC.NTR_and_ORCADES_pill_separated.txt",quote=F,sep="\t",row.names=F)
...
data1.mat2$cohort_by_family = cohort_by_family
save.image("../duoHMM_data_for_RSTAN.more_stringent.no_GPC.NTR_and_ORCADES_pill_separated.RData")


twin.datasets.no.art = rbind(ntr.counts.non.art[,c("CHILD","PARENT","n.crossovers","age.at.birth","cohort","other.twin")],qtr.counts.non.art[,c("CHILD","PARENT","n.crossovers","age.at.birth","cohort","other.twin")])

write.table(twin.datasets.no.art,"maternal_crossovers.informative_meioses.with_twins_indicated.QTR_and_NTR_only_nonART_mothers.txt",quote=F,sep="\t",row.names=F)
