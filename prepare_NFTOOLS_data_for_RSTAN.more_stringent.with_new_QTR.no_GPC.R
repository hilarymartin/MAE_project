...
"QTR/all_QTR_370K_samples.with_parental_age.txt","QTR/all_QTR_610K_samples.with_parental_age.txt","QTR/new_samples.CoreExome.with_parental_age.txt",
...
for(i in 1:length(all.events)){
    ...
    mat.events$age.at.birth = as.numeric(age[match(mat.events$child,age$individual),"maternal_age"])
    pat.events$age.at.birth = as.numeric(age[match(pat.events$child,age$individual),"paternal_age"])
...
mat.key.model1=mat.key.model1[order(mat.key.model1$Cohort.type),]
pat.key.model1=pat.key.model1[order(pat.key.model1$Cohort.type),]

write.table(mat.key.model1,"RSTAN_output/key_for_maternal_NFTOOLS_cohorts_to_include_in_model_1.more_stringent.with_new_QTR.no_GPC.txt",quote=F,sep="\t",row.names=F)
write.table(pat.key.model1,"RSTAN_output/key_for_paternal_NFTOOLS_cohorts_to_include_in_model_1.more_stringent.with_new_QTR.no_GPC.txt",quote=F,sep="\t",row.names=F)


data1.mat2=list(y=data1.mat$Freq,cohort=as.integer(as.factor(data1.mat$cohort)),family=as.integer(as.factor(data1.mat$family)),Age=as.numeric(data1.mat$age.at.birth),J=nrow(data1.mat),
I=length(unique(data1.mat$family)),    C=length(unique(data1.mat$cohort)))
data1.pat2=list(y=data1.pat$Freq,cohort=as.integer(as.factor(data1.pat$cohort)),family=as.integer(as.factor(data1.pat$family)),Age=as.numeric(data1.pat$age.at.birth),J=nrow(data1.pat),
I=length(unique(data1.pat$family)),    C=length(unique(data1.pat$cohort)))
...
save.image("NFTOOLS_data_for_RSTAN.more_stringent.with_new_QTR.no_GPC.RData")
