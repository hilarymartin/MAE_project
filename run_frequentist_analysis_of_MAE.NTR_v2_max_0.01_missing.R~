...
### test effect of grandparental age in 
...
qtr.age=read.delim("QTR/QTR_parental_and_grandparental_age.2014_survey.txt",header=T,stringsAsFactors=F,na="")

qtr.age$grandmat_age=-as.double(as.Date(qtr.age$mat_grandma_DOB,format="%d/%m/%Y")-as.Date(qtr.age$mat_DOB,format="%d/%m/%Y"))/365
qtr.age$grandpat_age=-as.double(as.Date(qtr.age$mat_grandpa_DOB,format="%d/%m/%Y")-as.Date(qtr.age$mat_DOB,format="%d/%m/%Y"))/365

qtr.age$grandmat_age[is.na(qtr.age$grandmat_age)] = qtr.age$mat_grandma_AGE[is.na(qtr.age$grandmat_age)]
qtr.age$grandpat_age[is.na(qtr.age$grandpat_age)] = qtr.age$mat_grandpa_AGE[is.na(qtr.age$grandpat_age)] 

qtr.age=qtr.age[!(is.na(qtr.age$grandmat_age)|is.na(qtr.age$grandpat_age)|qtr.age$grandmat_age<15|qtr.age$grandpat_age<15),]
fam.qtr610=read.delim("QTR/QTR.excl_missing_samples_and_one_MZ_per_pair.610K.fam",header=F,stringsAsFactors=F)
fam.qtr370=read.delim("QTR/QTR.excl_missing_samples_and_one_MZ_per_pair.370K.fam",header=F,stringsAsFactors=F)
fam=rbind(fam.qtr610,fam.qtr370)


qtr.610.mat.nftools=data1.mat[data1.mat$cohort %in% c("QTR_610K"),]
qtr.610.mat.nftools$mother = fam[match(qtr.610.mat.nftools$child,fam[,2]),4]
qtr.610.mat.nftools$maternal_grandmum_age=qtr.age[match(as.character(qtr.610.mat.nftools$mother),as.character(qtr.age$MUM_ID)),"grandmat_age"]
qtr.610.mat.nftools$maternal_granddad_age=qtr.age[match(as.character(qtr.610.mat.nftools$mother),as.character(qtr.age$MUM_ID)),"grandpat_age"]



###actually, none of the 370K families have grandparental age
qtr.370.mat.nftools=data1.mat[data1.mat$cohort %in% c("QTR_370K"),]
qtr.370.mat.nftools$mother = fam[match(qtr.370.mat.nftools$child,fam[,2]),4]
qtr.370.mat.nftools$maternal_grandmum_age=qtr.age[match(as.character(qtr.370.mat.nftools$mother),as.character(qtr.age$MUM_ID)),"grandmat_age"]
qtr.370.mat.nftools$maternal_granddad_age=qtr.age[match(as.character(qtr.370.mat.nftools$mother),as.character(qtr.age$MUM_ID)),"grandpat_age"]

both.mat.nftools=rbind(qtr.610.mat.nftools[!is.na(qtr.610.mat.nftools$maternal_grandmum_age),c("Freq","age.at.birth","family","maternal_grandmum_age","maternal_granddad_age")],
    ntr.mat.nftools[!is.na(ntr.mat.nftools$maternal_grandmum_age),    c("Freq","age.at.birth","family","maternal_grandmum_age","maternal_granddad_age")])

summary(lme(Freq~age.at.birth+maternal_grandmum_age,~1|as.factor(family),data=both.mat.nftools))

summary(lme(Freq~age.at.birth+maternal_grandmum_age,~1|as.factor(family),data=qtr.610.mat.nftools[!is.na(qtr.610.mat.nftools$maternal_grandmum_age),]))
summary(lme(Freq~age.at.birth+maternal_granddad_age,~1|as.factor(family),data=qtr.610.mat.nftools[!is.na(qtr.610.mat.nftools$maternal_granddad_age),]))
...
pdf("/home/hilary/maternal_age_recombination/frequentist_analysis/scatterplots_of_ncrossovers_vs_maternal_grandmaternal_age.with_LMM_results.pdf",height=5,width=5)
plot(jitter(qtr.610.mat.nftools[!is.na(qtr.610.mat.nftools$maternal_grandmum_age),"maternal_grandmum_age"]),jitter(qtr.610.mat.nftools[!is.na(qtr.610.mat.nftools$maternal_grandmum_age),"Freq"]),
     xlab="mother's mother's age at birth",     ylab="number of crossovers",main="QTR610",pch=19,col=alpha("black",0.5))
my.lme=summary(lme(Freq~maternal_grandmum_age,~1|as.factor(family),data=qtr.610.mat.nftools[!is.na(qtr.610.mat.nftools$maternal_grandmum_age),]))
abline(a=my.lme$tTable[1,1],b=my.lme$tTable[2,1])
    legend("topleft",paste0("slope = ",round(my.lme$coefficients$fixed[2],2)))
    legend("topright",paste0("p = ",round(my.lme$tTable[2,5],3)))
plot(jitter(ntr.mat.nftools[!is.na(ntr.mat.nftools$maternal_grandmum_age),"maternal_grandmum_age"]),jitter(ntr.mat.nftools[!is.na(ntr.mat.nftools$maternal_grandmum_age),"Freq"]),xlab="mother's mother's age at birth",
     ylab="number of crossovers",main="NTR",pch=19,col=alpha("black",0.5))
my.lme=summary(lme(Freq~maternal_grandmum_age,~1|as.factor(family),data=ntr.mat.nftools[!is.na(ntr.mat.nftools$maternal_grandmum_age),]))
abline(a=my.lme$tTable[1,1],b=my.lme$tTable[2,1])
    legend("topleft",paste0("slope = ",round(my.lme$coefficients$fixed[2],2)))
    legend("topright",paste0("p = ",round(my.lme$tTable[2,5],3)))
...
write.table(nftools.mat.adjusted.spearman.by.cohort,"frequentist_analysis/nftools.mat.adjusted.spearman.by.cohort.txt",quote=F,sep="\t")
write.table(duohmm.mat.adjusted.spearman.by.cohort,"frequentist_analysis/duohmm.mat.adjusted.spearman.by.cohort.txt",quote=F,sep="\t")
write.table(duohmm2.mat.adjusted.spearman.by.cohort,"frequentist_analysis/duohmm_in_nftools.mat.adjusted.spearman.by.cohort.txt",quote=F,sep="\t")

write.table(summary.nftools.mat.results.by.cohort,"frequentist_analysis/nftools.mat.lme.results.by.cohort.txt",quote=F,sep="\t")
write.table(summary.duohmm.mat.results.by.cohort,"frequentist_analysis/duohmm.mat.lme.results.by.cohort.txt",quote=F,sep="\t")
write.table(summary.duohmm2.mat.results.by.cohort,"frequentist_analysis/duohmm_in_nftools.mat.lme.results.by.cohort.txt",quote=F,sep="\t")

write.table(summary.nftools.mat.adjusted.results.by.cohort,"frequentist_analysis/nftools.mat.lm_on_adjusted_counts.by.cohort.txt",quote=F,sep="\t")
write.table(summary.duohmm.mat.adjusted.results.by.cohort,"frequentist_analysis/duohmm.mat.lm_on_adjusted_counts.by.cohort.txt",quote=F,sep="\t")
write.table(summary.duohmm2.mat.adjusted.results.by.cohort,"frequentist_analysis/duohmm_in_nftools.mat.lm_on_adjusted_counts.by.cohort.txt",quote=F,sep="\t")


write.table(nftools.pat.adjusted.spearman.by.cohort,"frequentist_analysis/nftools.pat.adjusted.spearman.by.cohort.txt",quote=F,sep="\t")
write.table(duohmm.pat.adjusted.spearman.by.cohort,"frequentist_analysis/duohmm.pat.adjusted.spearman.by.cohort.txt",quote=F,sep="\t")
write.table(duohmm2.pat.adjusted.spearman.by.cohort,"frequentist_analysis/duohmm_in_nftools.pat.adjusted.spearman.by.cohort.txt",quote=F,sep="\t")

write.table(summary.nftools.pat.results.by.cohort,"frequentist_analysis/nftools.pat.lme.results.by.cohort.txt",quote=F,sep="\t")
write.table(summary.duohmm.pat.results.by.cohort,"frequentist_analysis/duohmm.pat.lme.results.by.cohort.txt",quote=F,sep="\t")
write.table(summary.duohmm2.pat.results.by.cohort,"frequentist_analysis/duohmm_in_nftools.pat.lme.results.by.cohort.txt",quote=F,sep="\t")

write.table(summary.nftools.pat.adjusted.results.by.cohort,"frequentist_analysis/nftools.pat.lm_on_adjusted_counts.by.cohort.txt",quote=F,sep="\t")
write.table(summary.duohmm.pat.adjusted.results.by.cohort,"frequentist_analysis/duohmm.pat.lm_on_adjusted_counts.by.cohort.txt",quote=F,sep="\t")
write.table(summary.duohmm2.pat.adjusted.results.by.cohort,"frequentist_analysis/duohmm_in_nftools.pat.lm_on_adjusted_counts.by.cohort.txt",quote=F,sep="\t")
...
pdf("/home/hilary/maternal_age_recombination/frequentist_analysis/box_plot_of_beta_Age_from_LMM_on_informative_nuclear_families_duoHMM.pdf",height=4,width=6)
...
pdf("/home/hilary/maternal_age_recombination/frequentist_analysis/box_plot_of_beta_Age_from_LMM_on_all_informative_duos_duoHMM.pdf",height=4,width=6)
...
pdf("/home/hilary/maternal_age_recombination/frequentist_analysis/box_plot_of_beta_Age_from_LMM_on_all_duos_from_fams_with_multiple_kids.duoHMM.pdf",height=4,width=6)
