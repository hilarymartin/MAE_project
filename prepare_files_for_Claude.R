...
keep = data[data$informative.2gen.2parents,]
write.table(keep,gsub(".txt",".informative_nuc_fams.txt",files.to.open[f]),sep="\t",quote=F)
all.data=rbind(all.data,data)
}

keep= all.data[all.data$informative.2gen.2parents,]

keep$duo = paste0(keep$CHILD,"-",keep$PARENT)
