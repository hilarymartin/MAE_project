...
out.prefix = args[2]#"FVG"
out.suffix = args[3]#"FVG"
recomb.file=args[1]#"FVG/duoHMM_results/FVG.all.more_stringent.generr_removed.recombinations.p_0.5.annotated.double_xovers_within_X_SNPs_removed.txt"
...
duoHMM.all=duoHMM.all[!is.na(duoHMM.all$sex ),]
duoHMM.all=duoHMM.all[duoHMM.all$sex =="Female",]
...
#sizes=c(max(duoHMM.all$END-duoHMM.all$START) + 1)
sizes=c(30000)
...
write.table(overlap.stats,paste0("hotspot_overlap/",out.prefix,".",out.suffix,".female.overlap_with_hotspots.txt"),quote=F,sep="\t")
