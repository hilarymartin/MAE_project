...
if(argv[1]==1){
    load("RSTAN_output_on_duoHMM_more_stringent/with_new_QTR.no_GPC_different_link_for_NB_changing_sigmasq_m_prior/RSTAN.model3.beta_age_by_cohort.adjusted_priors.all_chains.mat.including_alpha.RData")
    mysim<-extract(model3.mat,permuted=T)
    y=data2.mat2$y
    cohort=data2.mat2$cohort
    simple.cohort=data2.mat2$cohort2
    ...
    my.model=3
    J=length(y)
    mydir="RSTAN_output_on_duoHMM_more_stringent/with_new_QTR.no_GPC_different_link_for_NB_changing_sigmasq_m_prior/model3"
    ...
    load("RSTAN_output_on_duoHMM_more_stringent/with_new_QTR.no_GPC_different_link_for_NB_changing_sigmasq_m_prior/RSTAN.model3.beta_age_by_cohort.adjusted_priors.all_chains.pat.including_alpha.RData")
    mysim<-extract(model3.pat,permuted=T)
    y=data2.pat2$y
    cohort=data2.pat2$cohort
    simple.cohort=data2.pat2$cohort2
    ...
    my.model=3.2
#remove two FVG children who appear to be MZ twins and who have a very low # of crossovers (1 and 2)
    remove = which(y<3)
    y=y[-remove]
    cohort=cohort[-remove]
    bad.mother=unique(mother[remove])
    mother=mother[-remove]
    mother[mother >bad.mother] =     mother[mother >bad.mother]-1 
    Age=Age[-remove]
...
#### Model 1.5 -- negative binomial model for informative families only
   load("RSTAN_output_on_duoHMM_more_stringent/with_new_QTR.no_GPC_different_link_for_NB_changing_sigmasq_m_prior/RSTAN.modell1.5.all_chains.maternal.mu_m_N_3.7_0.2.sigmasq_m_IG_3_0.01.including_alpha.RData")
   ...
   cohort.by.family=data1.mat2$cohort_by_family
    ...
    my.model=1.5
    J=length(y)
    mydir="RSTAN_output_on_duoHMM_more_stringent/with_new_QTR.no_GPC_different_link_for_NB_changing_sigmasq_m_prior/model1.5"
...
#### Model 1.5 -- negative binomial model for informative families only
    load("/well/donnelly/hilary/maternal_age_and_recombination/RSTAN_output_on_duoHMM_more_stringent/with_new_QTR.no_GPC_different_link_for_NB_changing_sigmasq_m_prior/RSTAN.modell1.5.all_chains.paternal.mu_m_N_3.2_0.2.sigmasq_m_IG_3_0.01.including_alpha.RData")
...
#### Model 1.6 -- beta_Age drawn from distribution; alphas drawn from N(mu_cohort,sigmasq)
    ...
    load("/well/donnelly/hilary/maternal_age_and_recombination/RSTAN_output_on_duoHMM_more_stringent/with_new_QTR.no_GPC_different_link_for_NB_changing_sigmasq_m_prior/RSTAN.modell1.6.all_chains.mat.including_alpha.mu_m_N_41_100.sigmasq_m_IG_2_40.RData")
    ...
    my.model=1.6
    J=length(y)
    mydir="RSTAN_output_on_duoHMM_more_stringent/with_new_QTR.no_GPC_different_link_for_NB_changing_sigmasq_m_prior/model1.6"
    ...
    load("/well/donnelly/hilary/maternal_age_and_recombination/RSTAN_output_on_duoHMM_more_stringent/with_new_QTR.no_GPC_different_link_for_NB_changing_sigmasq_m_prior/RSTAN.modell1.6.all_chains.pat.including_alpha.mu_m_N_27_64.sigmasq_m_IG_2_15.RData")
...
#          return(rnbinom(1,size=exp(0.1*(a0[i]+beta_Age[cohort[i]]*age[i]))/(omega-1),prob=1/omega))
      ...
      a0=family.effects[family]
      draws=sapply(1:N,function(i){
#          return(rnbinom(1,size=exp(0.1*(a0[i]+beta_Age[cohort2[i]]*age[i]))/(omega-1),prob=(1/(p_by_cohort[cohort[i]] * (omega-1) + 1))))
                    return(rnbinom(1,size=exp((a0[i]+beta_Age[cohort2[i]]*age[i]))/(omega-1),prob=(1/(p_by_cohort[cohort[i]] * (omega-1) + 1))))
      })
      return(draws)
  }else if(model==3.2){
      family.effects=rnorm(n=length(unique(family)),mean=mu_m,sd=sqrt(sigmasq_m))
...
#    par(mfrow=c(3,3))
 #   par(mar=c(4,6,4,1),mfrow=c(3,3),oma=c(2,1,2,0))
   par(mar=c(5,4,2,2),mfrow=c(3,3),oma=c(2,2,2,0))
    ...
    pdf(paste(mydir,"/","ecdfs_of_observed_and_simulated_y_from_highest_lp_estimates.by_cohort.",description,".pdf",sep=""),height=12,width=24)
 #   par(mfrow=c(4,7))
#    par(mar=c(2,2,2,1),mfrow=c(4,7),oma=c(2,2,2,0))
par(mfrow=c(4,8),oma=c(2,2,2,0),mar=c(4,4,2,2))
...
if(my.model==1){
    ...
    y.predict.best <- predict_y(a0=best.draw$exp_a0[mother],beta_Age=best.draw$beta_Age,age=Age,cohort=cohort,p_by_cohort=best.draw$p_by_cohort,model=my.model,cohort2=simple.cohort)
}else if(my.model==3.2){
...
if(my.model<3){
  ...
  pdf(paste(mydir,"/","residual_plot_vs_age.",description2,".by_cohort.pdf",sep=""),height=12,width=24,useDingbats=F)
#    par(mar=c(4,6,4,1),mfrow=c(4,7),oma=c(2,1,2,0))
par(mfrow=c(4,8),oma=c(2,2,2,0),mar=c(5,4,2,2))
}
mylabels=c(paste(c("2 generations, >2 children","3 generations, 2 children","2 generations, 2 children"),"both parents",sep=", "),paste(c("2 generations, >2 children","3 generations, 2 children","2 generations, 2 children"),"1 parent",sep=", "))
names(mylabels)=c("infor.2gen.2parents","infor.3gen.2parents","noninfor.2kids.2gen.2parents","infor.2gen.1parent","infor.3gen.1parent","noninfor.2kids.2gen.1parent")
...
if(my.model<3){
    ...
    pdf(paste(mydir,"/","residual_plot.by_cohort.",description2,".pdf",sep=""),height=12,width=24)
#    par(mfrow=c(4,7))
    par(mar=c(4,6,4,1),mfrow=c(4,8),oma=c(2,1,2,0))
