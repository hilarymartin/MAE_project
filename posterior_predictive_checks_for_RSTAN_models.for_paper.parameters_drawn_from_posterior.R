...
if(argv[1]==1){
    ...
    my.model=3
    J=length(y)
    mydir="RSTAN_output_on_duoHMM_more_stringent/posterior_predictive_checks_v2/model3"
    ...
    my.model=3.2
    J=length(y)
    mydir="RSTAN_output_on_duoHMM_more_stringent/posterior_predictive_checks_v2/model3.2"
...
#### Model 1.5 -- negative binomial model for informative families only
   ...
   cohort.by.family=data1.mat2$cohort_by_family
    ...
    my.model=1.5
    J=length(y)
    mydir="RSTAN_output_on_duoHMM_more_stringent/posterior_predictive_checks_v2/model1.5"
...
#### Model 1.6 -- beta_Age drawn from distribution; alphas drawn from N(mu_cohort,sigmasq)
    ...
    my.model=1.6
    J=length(y)
    mydir="RSTAN_output_on_duoHMM_more_stringent/posterior_predictive_checks_v2/model1.6"
...
#### Model 1.6.2 -- common beta_Age; alphas drawn from N(mu_cohort,sigmasq)
    ...
    my.model=1.62
    J=length(y)
    mydir="RSTAN_output_on_duoHMM_more_stringent/posterior_predictive_checks_v2/model1.62"
...
#### Model 1.5.4 -- negative binomial model for informative families only, no cohort effect but family effects drawn from distribution with cohort-specific mean; common beta_Age
    ...
    my.model=1.54
    J=length(y)
    mydir="RSTAN_output_on_duoHMM_more_stringent/posterior_predictive_checks_v2/model1.54"
    best.draw=sapply(1:(length(mysim)-1),function(i){
        x=mysim[[i]]
        if(class(x)=="array"){
            return(x[which.max(mysim$lp__)])
...
actual.best.draw = best.draw
random.draws = sample(length(mysim$lp__),1000,replace=F)
for(i in 1:1000){
  d=random.draws[i]
    best.draw=sapply(1:(length(mysim)-1),function(i){
        x=mysim[[i]]
...
best.draw = actual.best.draw
              
pdf(paste(mydir,"/","ecdfs_of_observed_and_simulated_y_from_random_posterior_draws.",description,".pdf",sep=""),height=5,width=5)
...
if(my.model<3){
    pdf(paste(mydir,"/","ecdfs_of_observed_and_simulated_y_from_random_posterior_draws.by_cohort.",description,".pdf",sep=""),height=10,width=10)
#    par(mfrow=c(3,3))
    par(mar=c(4,6,4,1),mfrow=c(3,3),oma=c(2,1,2,0))
} else {
    pdf(paste(mydir,"/","ecdfs_of_observed_and_simulated_y_from_random_posterior_draws.by_cohort.",description,".pdf",sep=""),height=12,width=21)
 #   par(mfrow=c(4,7))
...
pdf(paste(mydir,"/","histograms_of_observed_and_simulated_y_from_random_posterior_draws.",description,".simulation1.pdf",sep=""),height=5,width=5)
...
pdf(paste(mydir,"/","histograms_of_observed_and_simulated_y_from_random_posterior_draws.",description,".pdf",sep=""),height=20,width=20)
obs.hist=hist(y,plot=F)
par(mfrow=c(4,5))
for(i in 1:20){
    sim.hist=hist(best.draw.sims[i,],plot=F)
...
#pdf(paste(mydir,"/","histogram_of_summary_statistics.random_posterior_draws.",description,".pdf",sep=""),height=10,width=15)
pdf(paste(mydir,"/","histogram_of_summary_statistics.random_posterior_draws.",description,".pdf",sep=""),height=10,width=10)
#par(mfrow=c(2,3))
#par(mfrow=c(2,2))
...
mydescriptions=c(paste0("random_posterior_draws.",description))
