...
data1.mat=data1.mat[data1.mat$cohort %in% names(mat.cohort.count[mat.cohort.count>20]) & !data1.mat$cohort=="GPC",]
data1.pat=data1.pat[data1.pat$cohort %in% names(pat.cohort.count[pat.cohort.count>20]) & !data1.pat$cohort=="GPC",]
...
write.table(mat.key.model1,"RSTAN_output/key_for_maternal_NFTOOLS_cohorts_to_include_in_model_1.more_stringent.no_GPC.txt",quote=F,sep="\t",row.names=F)
write.table(pat.key.model1,"RSTAN_output/key_for_paternal_NFTOOLS_cohorts_to_include_in_model_1.more_stringent.no_GPC.txt",quote=F,sep="\t",row.names=F)
...
save.image("NFTOOLS_data_for_RSTAN.more_stringent.no_GPC.RData")
