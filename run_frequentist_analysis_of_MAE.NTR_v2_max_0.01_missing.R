...
data0=read.delim("NTR/duoHMM_results/NTR_v2_minus_MZs.all.post_MERLIN.max_0.01_missing.generr_removed.recombination_count.p_0.5.annotated.double_xovers_within_X_SNPs_removed.informative_nuclear_families_only.txt",header=T)
data1=read.delim("NTR/duoHMM_results/NTR_v2_minus_MZs.all.post_MERLIN.max_0.01_missing.generr_removed.recombination_count.p_0.5.annotated.double_xovers_within_X_SNPs_removed.informative_meioses_multikid_families_only.txt",
    header=T)

data0$mat.age.at.birth=ntr.age[match(data0$CHILD,ntr.age$individual),"maternal_age"]
data0$pat.age.at.birth=ntr.age[match(data0$CHILD,ntr.age$individual),"paternal_age"]


data1$mat.age.at.birth=ntr.age[match(data1$CHILD,ntr.age$individual),"maternal_age"]
data1$pat.age.at.birth=ntr.age[match(data1$CHILD,ntr.age$individual),"paternal_age"]


summary(lme(nrec~mat.age.at.birth,~1|as.factor(PARENT),data=data0[data0$sex=="Female",]))
summary(lme(nrec~mat.age.at.birth,~1|as.factor(PARENT),data=data1[data1$sex=="Female",]))
