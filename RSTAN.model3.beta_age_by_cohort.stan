  data { 
    ...
    int<lower=0> C; // number of cohort/family types
    int<lower=0> C2; // number of cohorts
    int y[J]; // number of recombinations
    int family[J]; // indicates which mother is with each child
    vector[J] Age; //maternal age at birth
    int cohort[J];//which cohort/family-type 
    int cohort2[J];//which cohort 
  ...
  parameters {
    ...
    vector[C2] beta_Age;  //coefficient for age
  ...
  transformed parameters {
      ...
      for(j in 1:J){
        ...
        mu[j] <-  exp(0.1*(exp_a0_bychild[j]+beta_Age[cohort2[j]]*Age[j])) / (omega - 1); ///needed to keep in exp(0.1*...) to ensure that mu > 0, since shape parameter of neg binomial must be >0
      }
  }
  model {
