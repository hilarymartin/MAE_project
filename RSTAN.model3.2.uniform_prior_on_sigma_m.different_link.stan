  ...
  transformed parameters {
      ...
      sigmasq_m <- sigma_m * sigma_m;
      omega <- 1 / inv_omega;
      for(j in 1:J){
        exp_a0_bychild[j] <- exp_a0[family[j]];
        p[j] <- p_by_cohort[cohort[j]];
        mu[j] <-  exp(1*(exp_a0_bychild[j]+beta_Age*Age[j])) / (omega - 1); ///needed to keep in exp(0.1*...) to ensure that mu > 0, since shape parameter of neg binomial must be >0
      }
  }
  model {
    beta_Age ~ normal(0,0.01); ///smaller variance since the age effect is now multiplicative: baseline * exp(0.1* beta_Age * age)
