...
write.table(all.data,"all_cohorts.post_MERLIN.generr_removed.recombination_count.p_0.5.annotated.double_xovers_within_X_SNPs_removed.more_stringent.txt",quote=F,sep="\t",row.names=F)
...
write.table(mat.key.model0,"RSTAN_output/key_for_maternal_cohorts_to_include_in_model_1.informative_nuclear_families_only.more_stringent.txt",quote=F,sep="\t",row.names=F)
write.table(pat.key.model0,"RSTAN_output/key_for_paternal_cohorts_to_include_in_model_1.informative_nuclear_families_only.more_stringent.txt",quote=F,sep="\t",row.names=F)
...
write.table(mat.key.model1,"RSTAN_output/key_for_maternal_cohorts_to_include_in_model_1.more_stringent.txt",quote=F,sep="\t",row.names=F)
write.table(pat.key.model1,"RSTAN_output/key_for_paternal_cohorts_to_include_in_model_1.more_stringent.txt",quote=F,sep="\t",row.names=F)
...
write.table(mat.key.model2,"RSTAN_output/key_for_maternal_cohorts_to_include_in_model_2.more_stringent.txt",quote=F,sep="\t",row.names=F)
write.table(pat.key.model2,"RSTAN_output/key_for_paternal_cohorts_to_include_in_model_2.more_stringent.txt",quote=F,sep="\t",row.names=F)
...
pat.beta.parameters=read.delim("parameters_for_beta_distribution.model3_paternal.more_stringent.txt",header=T)[,1:5]
...
data2.mat2=list(y=data2.mat$nrec,cohort=as.integer(as.factor(data2.mat$cohort.family.type)),family=as.integer(as.factor(data2.mat$PARENT)),Age=as.numeric(data2.mat$age.at.birth),J=nrow(data2.mat),
    ...
    beta_parameters=c(mat.beta.parameters[order(mat.beta.parameters$Code),"beta"]))
data2.pat2=list(y=data2.pat$nrec,cohort=as.integer(as.factor(data2.pat$cohort.family.type)),family=as.integer(as.factor(data2.pat$PARENT)),Age=as.numeric(data2.pat$age.at.birth),J=nrow(data2.pat),
    ...
    beta_parameters=pat.beta.parameters[order(pat.beta.parameters$Code),"beta"])

save.image("duoHMM_data_for_RSTAN.more_stringent.RData")
